...
public class JdbcProductRepository implements ProductRepository {
    ...
    /**
     * Создаёт репозиторий продуктов на основе переданного пула соединений.
     *
     ...
     * <p>
     * Если у продукта отсутствует ID — выполняется вставка (insert).
     * Если ID присутствует — выполняется обновление (update).
     *
     ...
     * @return найденный продукт или {@code null}, если продукт отсутствует
    ...
    /**
     * Выполняет вставку нового продукта в БД.
     * <p>
     * После успешного выполнения устанавливает ID, полученный через `RETURNING`.
     *
     ...
     *
     * @param product продукт с обновлёнными данными
     * @throws RuntimeException при ошибках SQL
     */
    ...
    /**
     * Заполняет общие поля PreparedStatement,
     * используемые как в INSERT, так и в UPDATE.
     *
     * @param ps      подготовленный SQL-запрос
     * @param product продукт, из которого берутся данные
     * @throws SQLException если установка параметров завершается ошибкой
     */
    ...
    /**
     * Преобразует строку результата SQL-запроса в объект {@link Product}.
     *
     * @param rs результирующий набор данных
     * @return продукт, соответствующий строке результата
